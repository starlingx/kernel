...
BuildRequires: bzip2, xz, findutils, gzip, m4, perl-interpreter, perl-Carp, perl-devel, perl-generators, diffutils, gawk
...
%global perf_make \
  make -s EXTRA_CFLAGS="${RPM_OPT_FLAGS}" LDFLAGS="%{__global_ldflags}" %{?cross_opts} -C tools/perf V=1 NO_PERF_READ_VDSO32=1 NO_PERF_READ_VDSOX32=1 WERROR=0 NO_LIBUNWIND=1 HAVE_CPLUS_DEMANGLE=1 NO_GTK2=1 NO_STRLCPY=1 NO_BIONIC=1 prefix=%{_prefix} PYTHON=%{__python}
%if %{with_perf}
# perf
...
- Place module signing keys in a separate package kernel-devel-keys.

* Mon Jul 05 2021 Jiping Ma <jiping.ma2@windriver.com> - 5.10.30
- This spec file is based on
- https://vault.centos.org/altarch/7/experimental/Source/SPackages/kernel-5.10.23-200.el7.src.rpm
- kernel.spec file with adaptations for StarlingX and linux-yocto.
- Added STX patches
- Support linux-yocto git source
- Copy keys as part of the devel package
- kernel-5.10.30-x86_64.config.tis_extra repace of kernel-local.
- Add a dist field to avoid undesired rebuildsi
