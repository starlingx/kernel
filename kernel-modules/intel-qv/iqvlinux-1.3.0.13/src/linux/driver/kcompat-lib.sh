...
# tell if given funcion is exported and available for use by modules
# return 0 if given funcion is exported, 1 otherwise
# inputs:
# $1 - function to check
function is_exported() {
   if [ -n "$KSRC" ]; then
       MODULE_SYMVERS_DIR=$KSRC
   else
       MODULE_SYMVERS_DIR=/lib/modules/$(uname -r)/build
   fi
   grep -q "$1" $MODULE_SYMVERS_DIR/Module.symvers
}